...
const handlePlaceOrder = async () => {
  ...
  if (paymentMethod === "cod") {
    try {
      ...
      if (res.ok) {
        // --- UI UPDATES ---
        setCartItems([]); // Clears checkout summary items instantly
        window.dispatchEvent(new Event("cartUpdated")); // Signal Navbar to show 0
    ...
    try {
      ...
      if (res.ok && data.url) {
        // --- UI UPDATES ---
        await clearUserCart(); // Deletes from Backend
        setCartItems([]);      // Clears checkout summary items instantly
        window.dispatchEvent(new Event("cartUpdated")); // Signal Navbar to show 0
