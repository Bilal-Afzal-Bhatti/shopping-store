  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        ...
        // ✅ CRITICAL FIX: Extract message string instead of setting error object directly
        setErrorMessage(err.response?.data?.message || err.message || "Failed to fetch orders");
        setOrders([]);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Error Alert Banner */}
        {errorMessage && (
          <div className="mb-6 p-4 border-2 border-black bg-red-100 text-red-700 font-bold text-xs uppercase">
            {errorMessage}
          </div>
        )}
                ...
                <div className="p-4 sm:p-6">
                  ...
                  <div className="flex flex-wrap justify-between items-center gap-4 mb-6 border-b-2 border-gray-100 pb-4">
                    <div className="flex items-center gap-3">
                      <div className={`px-4 py-1 text-[10px] font-black uppercase italic ${
                        ...
                        {typeof order.orderStatus === 'string' ? order.orderStatus : 'processing'}
                      </div>
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-tighter">
                        ID: {String(order._id || '').slice(-10)}
                      </span>
                    </div>
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-8">
                    ...
                    <div className="w-full md:w-2/3">
                      <p className="text-[10px] text-gray-400 font-black uppercase mb-3 tracking-widest">
                        ...
                        >
                          {order.items?.map((item: any, i: number) => {
                            const backendBaseUrl = axiosInstance.defaults.baseURL
                              ? new URL(axiosInstance.defaults.baseURL).origin
                              : 'https://shopping-store-blond-one.vercel.app';

                            const imageUrl = item.image?.startsWith('http')
                              ? item.image
                              : `${backendBaseUrl}${item.image || ''}`;

                            // ✅ Safe extraction for item name (handles cases where name or color is an object)
                            const itemName = typeof item.name === 'object' ? item.name?.name || 'Product' : String(item.name || 'Product');
