  ...
  const handleRateProduct = async (ratingValue: number) => {
    ...
    try {
      const { data } = await axiosInstance.post(
        `/customer/products/${product._id}/rate`,
        { rating: ratingValue }
      );
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-0.5">
              {[1, 2, 3, 4, 5].map((star) => (
                ...
                >
                  <Star size={18}
                    fill={star <= displayStar ? "#FBBF24" : "none"}
                    stroke={star <= displayStar ? "#FBBF24" : "#D1D5DB"}
                    strokeWidth={1.5}
                  />
                </button>
              ))}
          ...
          <div className="flex items-center gap-3">
            ...
            {product.discount && product.discount !== "No Discount" && (
              <span className="bg-[#DB4444] text-white text-xs font-bold px-2.5 py-1 rounded-md">
                {product.discount}
              </span>
            )}
          </div>
